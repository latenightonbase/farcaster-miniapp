...
const YouTubeLivestreamFetcher: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const channelId = await getChannelIdFromHandle('@NickEh30_VODs');
    ...
    <div className="max-w-6xl mx-auto p-6 text-white">
      ...
      {loading && (
        <div className="text-center py-12">
          <p className="text-gray-400">Loading livestreams...</p>
        </div>
      )}
      ...
      {!loading && liveStreams.length > 0 && (
        <div className="mb-8">
          ...
          {liveStreams.map((stream) => (
            <div key={stream.id} className="mb-6 bg-red-800/50 p-4 rounded-lg">
              <iframe
                width="100%"
                height="360"
        ...
        <div className="relative">
          ...
          >
            {pastStreams.map((stream) => (
              <div
                key={stream.id}
                className="min-w-[150px] bg-gray-800 border border-gray-700 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow"
              >
                <div className="relative">
                  <a
                    href={stream.url}
